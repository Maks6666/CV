...
while ret:
    ...
    if person_coordinates:
        ...
        if len(person_coordinates) > 3:
            cv2.rectangle(frame, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 0, 255), 2)
            cv2.circle(frame, (x_center, y_center + 100), 3, (0, 0, 255), thickness=cv2.FILLED)
            cv2.putText(frame, "Detected group", (int(x_min), int(y_min)-10),
